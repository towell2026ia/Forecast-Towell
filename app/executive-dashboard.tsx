...
type Point = { period: string; label: string; phase: "history" | "current" | "future"; client: number | null; towell: number | null; ml: number | null; sale: number | null; order: number | null; delivery: number | null; wape: number | null; p50?: number | null; p90?: number | null; p95?: number | null };
const mlData=mlPayload as {champion:{wape:number;bias:number;model:string};forecast:{series_id:string;forecast:{period:string;value:number}[]}[]};
const mlTotal=mlData.forecast.find((row)=>row.series_id==="total-fendi-bd")?.forecast??[];
...
].map(([period,label,phase,client,towell,sale,order,delivery,wape]) => ({ period,label,phase,client,towell,ml:null,sale,order,delivery,wape,p50:towell,p90:towell ? Number(towell)*1.12 : null,p95:towell ? Number(towell)*1.2 : null } as Point));
...
export default function ExecutiveDashboard({ onGo, periodState }: { onGo:(id:ModuleId)=>void; periodState:string }) {
  ...
  const scoped = useMemo(() => base.map((point) => ({...point,ml:mlTotal.find((item)=>item.period===point.period)?.value?Math.round(mlTotal.find((item)=>item.period===point.period)!.value*factor):null,...Object.fromEntries(series.map(({key}) => [key,point[key] == null ? null : Math.round(point[key]! * factor)]))})), [factor]);
