...
export default function StatisticalEngineView({ supabaseConfigured }: { supabaseConfigured: boolean }) {
  ...
  const chart = useMemo(() => {
    ...
    const recent = selected.history.slice(-18);
    const last = recent.at(-1);
    return [...recent, ...(last ? [{ period: last.period, actual: null, forecast: last.actual }] : []), ...selected.forecast];
  }, [selected]);
  ...
  return <div className="space-y-6">
    ...
    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex gap-3"><AlertTriangle className="mt-0.5 size-5 shrink-0 text-amber-700"/><div><p className="text-sm font-semibold text-amber-950">Ejecución reproducible · universo FENDI auditado</p><p className="mt-1 text-sm leading-6 text-amber-800">Se recalculó desde registros normalizados de Walmart y familia FENDI únicamente. Venta y Pedido están disponibles como objetivos separados; 2023 queda fuera por falta de equivalencia aprobada.</p></div></div></div>
